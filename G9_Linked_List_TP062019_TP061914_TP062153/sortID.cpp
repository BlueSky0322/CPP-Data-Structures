...
void sortID()
{
	...
	else//many nodes in list
	{
		...
		while (current != NULL)//Traverse through linked list
		{
			...
			//while ID less than previous node AND head not reached, keep swapping
			while ((current->prevAddress != NULL) && (current->tutID < current->prevAddress->tutID))
			{				
