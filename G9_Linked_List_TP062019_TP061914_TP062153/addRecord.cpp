...
void addRecord()
{
	...
	//Create new node
	Tutor* newnode = CreateNewNode(id, name, dateJoin, dateTerm, hourlyRate, phone, address,
		tcCode, tcName, subCode, subName, rating);

	InsertToLinkedListEnd(newnode);//Insert to end of list

	cout << "Tutor Added. Enter 1 to add another tutor 2 to go back, and any other key to exit: ";
	cin >> cont;
	if (cont == 1)//add more records
	{
		addRecord();
	}
	else if (cont == 2)//exit to login screen
	{		
		cin.clear();
		cin.ignore(100000, '\n');
		login();
